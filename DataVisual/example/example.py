...
def motion_update_wheel_input(_vel_left_wheel, _vel_right_wheel, _theta, _omega, _dt,
                              prev_state):  # IMU inputs in robot frame
...
if __name__ == "__main__":
    ...
    for k in range(1, len(t)):
        ...
        if USE_GPS_AS_INPUT:
            ...
            x_predicted, F = motion_update_wheel_input(v_left_wheel[wheel_counter], v_right_wheel[wheel_counter],
                                                       theta_imu[euler_counter], omega[imu_counter], dt, x_est[k - 1])
            P_predicted = np.matmul(np.matmul(F, P_est[k - 1]), np.transpose(F)) + Q

        else:
            # IMU BASED MODEL
            x_predicted, F = motion_update_imu_input(a_x[imu_counter], a_y[imu_counter], theta_imu[euler_counter],
                                                     omega[imu_counter], dt, x_est[k - 1])
            P_predicted = np.matmul(np.matmul(F, P_est[k - 1]), np.transpose(F)) + Q
        ...
        if USE_GPS_FOR_CORRECTION and not USE_GPS_AS_INPUT:
            if gps_counter != prev_gps_counter:  # Use GPS data only if new
                ...
                x_predicted, P_predicted = measurement_update_gps(gps_x[gps_counter], gps_y[gps_counter], P_predicted,
                                                                  x_predicted)
        ...
        if USE_WHEEL_FOR_CORRECTION and not USE_WHEEL_AS_INPUT and not USE_GPS_AS_INPUT:
            if wheel_counter != prev_wheel_counter:
                ...
                x_predicted, P_predicted = measurement_update_wheel(v_left_wheel[wheel_counter],
                                                                    v_right_wheel[wheel_counter], P_predicted,
                                                                    x_predicted)
    ...
    ###### PLOT DELIVERABLES #########################################################################################
    utils.export_to_kml(x_est[:, 0], x_est[:, 1], x_true_arr, y_true_arr, LABEL_ESTIMATION_TYPE, "Ground Truth",
                        FILE_DATE)
    utils.save_results(x_est, P_est, x_true_arr, y_true_arr, theta_true_arr, t, f"{FILE_DATE}_{LABEL_ESTIMATION_TYPE}")
    utils.plot_position_comparison_2D(x_est[:, 0], x_est[:, 1], x_true_arr, y_true_arr, LABEL_ESTIMATION_TYPE,
                                      "Ground Truth", FILE_DATE)
